...
if(loadstuff){
      ...
      treefile<- "C.16/c_16/C.16STEXP50ML_combined.trees"

      localTreesDirectory = "C.16/marta/UCLD_Skyline/Tree_extractions"
      allTrees = scan(file=treefile, what="", sep="\n", quiet=T)
      burnIn = 0
      randomSampling = FALSE
      nberOfTreesToSample = 100
      mostRecentSamplingDatum = 2021.304109589041 #use lubridate, decimal_date(as.Date("2021-04-22"))
      ...
      treefile<- "C.16/c_16/C.16STEXP50ML_combined.tree"

      source("C.16/visualization/mccExtractions.r")
      mcc_tre = readAnnotatedNexus(treefile)
      mcc_tab = mccTreeExtraction(mcc_tre, mostRecentSamplingDatum)
      write.csv(mcc_tab, "C.16/c_16/C16_cluster_MCC.csv", row.names=F, quote=F)
      ...
      library(rgdal)
      my_spdf <- readOGR( 
        dsn= paste0("C.16/visualization/TM_WORLD_BORDERS_SIMPL-0.3/") , 
      ...
      reloadborders<-TRUE
      if(reloadborders){
          template_raster = raster("C.16/visualization/C16_studyarea_round2.asc")
          #borders = crop(getData("GADM", country="AFR", level=1), extent(template_raster))
          ...
          save(borders,file="borders2.Rdata")
      }
      load("borders2.Rdata") #borders
...
#mcc_tab2$startYear2<-date_decimal(mcc_tab[,"startYear"])
...
pdf('C.16/c_16/map_C16_ucld.pdf',width=8, height=10,bg="white")
...
for (i in 1:dim(mcc_tab)[1])
{
  ...
  curvedarrow(cbind(mcc_tab[i,"startLon"],mcc_tab[i,"startLat"]), cbind(mcc_tab[i,"endLon"],mcc_tab[i,"endLat"]), arr.length=NA,
              arr.width=NA, lwd=2.5*1.1, lty=1, lcol="grey22", arr.col="grey22", arr.pos=FALSE, curve=0.3, dr=NA, endhead=F)
  curvedarrow(cbind(mcc_tab[i,"startLon"],mcc_tab[i,"startLat"]), cbind(mcc_tab[i,"endLon"],mcc_tab[i,"endLat"]), arr.length=NA,
              arr.width=NA, lwd=2*1.1, lty=1, lcol=endYears_colours[i], arr.col="grey22", arr.pos=FALSE, curve=0.3, dr=NA, endhead=F)
}
